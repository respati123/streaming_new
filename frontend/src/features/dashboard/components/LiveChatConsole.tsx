      ...
      >
        {isLoading ? (
          ...
          messages.map((msg, idx) => (
            <div
              key={msg.id || idx}
              className="p-2.5 bg-white rounded-lg border border-zinc-200/90 shadow-xs flex flex-col gap-1 hover:border-zinc-300 transition-colors"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 flex-wrap">
                  {msg.isOwner && (
                    <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-amber-50 text-amber-900 border border-amber-200 font-mono flex items-center gap-0.5">
